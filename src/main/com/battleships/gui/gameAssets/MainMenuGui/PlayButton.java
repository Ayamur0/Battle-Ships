...
public class PlayButton extends MainMenuButton {
    ...
    public PlayButton(GuiManager guiManager, Loader loader){
        ...
        super.guiTexts.add(new GUIText("Singleplayer", 2.5f, font, new Vector2f(singleplayer.getPositions().x-singleplayer.getScale().x/2+0.01f,singleplayer.getPositions().y-singleplayer.getScale().y/2+0.0199f), 0.12f, true, 0.0f, 0.1f, new Vector3f(1.0f,0.0f,0.0f), new Vector2f()));
        super.guiTexts.add(new GUIText("Multiplayer", 2.5f, font,new Vector2f(multiplayer.getPositions().x-multiplayer.getScale().x/2+0.01f,multiplayer.getPositions().y-multiplayer.getScale().y/2+0.0199f), 0.12f, true, 0.0f, 0.1f, new Vector3f(1.0f,0.0f,0.0f), new Vector2f()));
        super.guiTexts.add(new GUIText("Back", 3, font,new Vector2f(back.getPositions().x-back.getScale().x/2+0.01f,back.getPositions().y-back.getScale().y/2+0.01f), 0.12f, true, 0.0f, 0.1f, new Vector3f(1.0f,0.0f,0.0f), new Vector2f()));
