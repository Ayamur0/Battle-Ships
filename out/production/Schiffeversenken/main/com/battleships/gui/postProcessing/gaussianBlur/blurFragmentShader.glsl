...
void main(void){
    ...
    out_colour = vec4(0.0);
    //add color of pixels around the pixel to the pixel to be blurred
    //values that the colors are multiplied with were calculated by using a gaussian kernel calculator
    //these values indicate the weight that the color of each pixel has on the blur of the center pixel
